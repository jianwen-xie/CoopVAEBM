...
class DataSet(object):
    ...
    def get_batch(self):
        ...
        if not self.prefetch and np.mod(self.batch_idx, self.read_len) == 0:
            self._load_data()
    ...
    def _imread_file(self, file_name):
        ...
        img = np.array(Image.open(file_path).convert('RGB').resize((self.image_size, self.image_size)))
    ...
    def __len__(self):
        return len(self.imgList) if not self.is_mnist else 55000
