...
def linear_interpolator2(z, npairs=8, ninterp=8):
    num_z = z.shape[0]
    z_dim = z.shape[1]
...
def linear_interpolator(z, npairs=8, ninterp=8):
    ...
    line_points = np.expand_dims(np.linspace(0, 1, int(ninterp/2)), 0)
    ...
    for ip in xrange(npairs):
        ...
        l_z2 = z[np.newaxis, pair[2]]
        r_z2 = z[np.newaxis, pair[3]]

        temp1 = np.dot(line_points.transpose(), l_z) + np.dot(1 - line_points.transpose(), r_z)

        temp2 = np.dot(line_points.transpose(), l_z2) + np.dot(1 - line_points.transpose(), r_z2)

        temp = np.concatenate([temp1, temp2],axis=0)
        interp_z[ip * ninterp :(ip + 1) * ninterp] = temp

        #interp_z[(ip + 1) * int(ninterp/2):] = temp2
